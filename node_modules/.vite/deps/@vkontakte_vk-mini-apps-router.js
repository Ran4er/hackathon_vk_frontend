import {
  Link
} from "./chunk-BDUDOEGN.js";
import "./chunk-VXJN2YTB.js";
import {
  require_jsx_runtime
...
var isString = (tmp) => typeof tmp === "string";
var isPageObject = (path) => {
  return typeof path === "object" && "path" in path;
};
var isPageWithOptionsPath = (path) => {
...
function fillParamsIntoPath(path, params) {
  ...
  const paramInjector = (acc, param) => {
    ...
    invariant2(params == null ? void 0 : params[paramName], `Missing parameter ${paramName} while building route ${path}`);
...
function extractPathFromNavigationTarget(to, defaultPathname = "") {
  if (isString(to)) {
    return to;
  }
...
function transformSearchParams(searchParams = "") {
  if (!isString(searchParams) && !(searchParams instanceof URLSearchParams)) {
    return `${new URLSearchParams(searchParams)}`;
  }
  return searchParams.toString();
}
function getPathFromTo({ to, params, defaultPathname = "" }) {
  ...
  if (hasParams) {
    ...
    return isPageWithOptionsPath(to) ? createPath({ ...to, pathname: filledPath, search }) : filledPath;
  }
...
var TransactionExecutor = class _TransactionExecutor {
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  constructor() {
    this.transactions = [];
  }
  static getInstance() {
    if (!_TransactionExecutor.instance) {
      _TransactionExecutor.instance = new _TransactionExecutor();
    }
    return _TransactionExecutor.instance;
  }
  static get isRunSyncActive() {
    const transactionExecutor = _TransactionExecutor.getInstance();
    const hasMultipleTransactions = transactionExecutor.transactions.length > 1;
    const hasSingleMultiActionTransaction = transactionExecutor.transactions.length === 1 && transactionExecutor.transactions[0].isMultiAction;
...
function fillHistory(config, routeNavigator, context) {
  var _a, _b;
...
// node_modules/@vkontakte/vk-mini-apps-router/dist/services/EventBus.js
var EventBus = class _EventBus {
  ...
  static broadcast(contextName, args) {
    const eventBus = _EventBus.getInstance();
  ...
  static subscribe(contextName, callback) {
    const eventBus = _EventBus.getInstance();
    if (!eventBus.isEventExist(contextName)) {
      eventBus.subscriptions[contextName] = /* @__PURE__ */ new Set();
    }
...
// node_modules/@vkontakte/vk-mini-apps-router/dist/services/ContextThrottleService.js
var ContextThrottleService = class _ContextThrottleService {
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  constructor() {
    this.interval = 0;
    this.throttled = true;
  ...
  static getInstance() {
    if (!_ContextThrottleService.instance) {
      _ContextThrottleService.instance = new _ContextThrottleService();
    }
    return _ContextThrottleService.instance;
  }
  ...
  static triggerContextUpdate(contextName, newValue) {
    const throttledService = _ContextThrottleService.getInstance();
...
function RouterProvider({ router, children, notFound, hierarchy, notFoundRedirectPath, interval = 400, useBridge = true, throttled = true }) {
  var _a, _b;
  const [popout, setPopout] = (0, import_react3.useState)(null);
  const [viewHistory] = (0, import_react3.useState)(new ViewHistory());
  const [panelsHistory, setPanelsHistory] = (0, import_react3.useState)([]);
  const isPopoutShown2 = (_a = router.state.location.state) == null ? void 0 : _a[STATE_KEY_SHOW_POPOUT];
  const dataRouterContext = (0, import_react3.useMemo)(() => {
    const routeNavigator = new DefaultRouteNavigator(router, viewHistory, setPopout);
    return { router, routeNavigator, viewHistory };
  }, [router, viewHistory, setPopout]);
  const routeContext = (0, import_react3.useMemo)(() => getRouteContext(router.state, panelsHistory), [router.state, panelsHistory]);
  const dataPopoutContext = (0, import_react3.useMemo)(() => {
...
function useThrottledContext(context) {
  ...
  const [prevValue, setPrevValue] = (0, import_react4.useState)(contextValue);
  const [throttledValue, setThrottledValue] = (0, import_react4.useState)(contextValue);
  if (!contextName) {
    console.error("No context display name found");
    return [contextValue, null];
  }
  (0, import_react4.useEffect)(() => {
    const unsubscribe = EventBus.subscribe(contextName, (throttleValue, prevValue2) => {
      setThrottledValue(throttleValue);
      setPrevValue(prevValue2);
    });
    return () => unsubscribe();
  }, []);
  (0, import_react4.useEffect)(() => {
...
function useRouteNavigator() {
  const routerContext = (0, import_react5.useContext)(RouterContext);
...
// node_modules/@vkontakte/vk-mini-apps-router/dist/hooks/useHref.js
function useHref(to, { relative, params }) {
...
function useLinkClickHandler(to, { target, replace: replaceProp, preventScrollReset, relative, params }) {
...
var RouterLinkInner = ({ to, relative, replace, target, reloadDocument, params, onClick, ...rest }, ref) => {
  let absoluteHref;
  let isExternal = false;
  let navigationTarget = to;
  if (isString(to) && ABSOLUTE_URL_REGEX2.test(to)) {
    absoluteHref = to;
    if (isBrowser) {
  ...
  const href = useHref(navigationTarget, { relative, params });
...
var RouterLink = (0, import_react9.forwardRef)(RouterLinkInner);
...
function useFirstPageCheck() {
  const routerContext = (0, import_react11.useContext)(RouterContext);
...
function useEnableSwipeBack() {
  const consumerId = (0, import_react12.useRef)(null);
...
function useBlocker(blocker) {
  ...
  const [routeContext] = useThrottledContext(RouteContext);
  const unblocker = (0, import_react13.useRef)();
